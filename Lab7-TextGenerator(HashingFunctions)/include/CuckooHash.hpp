...
void CuckooHash<Key, Value>::insert(const Key& key, const Value& value) {
    ...
    size_t h1 = hash1(key, capacity_);
    size_t h2 = hash2(key, capacity_);

    if (table_.get(h1).occupied && *table_.get(h1).key == key) { 
        *table_.get(h1).value = value; 
        return; 
    }

    if (table_.get(h2).occupied && *table_.get(h2).key == key) { 
        *table_.get(h2).value = value; 
        return; 
    }
    ...
    for (size_t i = 0; i < maxKick_; ++i) {

        size_t h = useTable1 ? hash1(curKey, capacity_) : hash2(curKey, capacity_);

        auto& slot = table_.get(h);

        if (!slot.occupied) {
            slot.key = std::move(curKey);
            slot.value = std::move(curValue);
            slot.occupied = true;
            count_++;
            return;
        }

       std::swap(curKey, *slot.key);
       std::swap(curValue, *slot.value);

       useTable1 = !useTable1;
    }

    rehash();
    insert(curKey, curValue);
...
bool CuckooHash<Key, Value>::remove(const Key& key) {
    ...
    size_t table1 = hash1(key, capacity_);
    size_t table2 = hash2(key, capacity_);

    if(table_.get(table1).occupied && *table_.get(table1).key == key){
        table_.get(table1).occupied = false;
        table_.get(table1).key.reset();
        table_.get(table1).value.reset();
        count_--;
        return true;
    }
    if(table_.get(table2).occupied && *table_.get(table2).key == key){
        table_.get(table2).occupied = false;
        table_.get(table2).key.reset();
        table_.get(table2).value.reset();
        count_--;
        return true;
    }
...
Value* CuckooHash<Key, Value>::find(const Key& key) {
    ...
    if(table_.get(table1).occupied && *table_.get(table1).key == key){
        return &(*table_.get(table1).value);
    }
    if(table_.get(table2).occupied && *table_.get(table2).key == key){
        return &(*table_.get(table2).value);
    }
...
Value& CuckooHash<Key, Value>::operator[](const Key& key) {
    ...
    if (table_.get(h1).occupied && *table_.get(h1).key == key)
        return *table_.get(h1).value;
    if (table_.get(h2).occupied && *table_.get(h2).key == key)
        return *table_.get(h2).value;
 
    insert(key, Value{});
    
    h1 = hash1(key, capacity_);
    h2 = hash2(key, capacity_);
...
void CuckooHash<Key, Value>::rehash() {
    ...
    for (size_t i = 0; i < oldCap; ++i) {
        if (oldTable.get(i).occupied) {
            insert(*oldTable.get(i).key, *oldTable.get(i).value);
        }
    }
}